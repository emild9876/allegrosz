...
from allegrosz.forms.item_forms import NewItemForm, EditItemForm, DeleteItemForm
...
def item(item_id):
    ...
    if db_item:
        delete_item = DeleteItemForm()
        return render_template('item.html', item=db_item, delete_item=delete_item)
...
def edit(item_id):
    ...
    if db_item:
        ...
        if form.validate_on_submit():
            ...
            conn.commit()
            flash(f'Item {form.title.data} has been successfully upgraded', 'success')
